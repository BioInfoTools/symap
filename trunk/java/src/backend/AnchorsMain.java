...
public class AnchorsMain
{
	...
	public boolean run(String proj1Name, String proj2Name) throws Exception
	{
		try
		{
			...
			annotBinSize = mProps.getInt("annot_bin");
			...
			//Hit.mergeOverlappingHits(diagHits); // too slow for big projects
			//Utils.incStat("DiagHitsMerged", diagHits.size());
			
			if (DEBUG)
			{
				p1.printAnnotIncExcCounts();
				p1.printBinStats();
				p2.printAnnotIncExcCounts();
				p2.printBinStats();
			}
...
/*			if (p1.isPseudo() && !isSelf)
			{
				// add fake annotations so that every hit has at least one annotation
				// this is kind of lame; there should be some better way to do this
				pool.executeUpdate("delete pseudo_annot.* from pseudo_annot, groups where groups.idx=pseudo_annot.grp_idx and type='hit' and groups.proj_idx=" + p1.idx);
				pool.executeUpdate("delete pseudo_annot.* from pseudo_annot, groups where groups.idx=pseudo_annot.grp_idx and type='hit' and groups.proj_idx=" + p2.idx);

				pool.executeUpdate("insert into pseudo_annot(select null,ph.grp1_idx,'hit','',ph.start1,ph.end1,ph.strand,'',ph.idx " +
						" from pseudo_hits as ph left join pseudo_hits_annot as pha on pha.hit_idx=ph.idx where pha.hit_idx is null and ph.pair_idx=" + pairIdx + ")");

				pool.executeUpdate("insert into pseudo_annot(select null,ph.grp2_idx,'hit','',ph.start1,ph.end1,ph.strand,'',ph.idx " +
						" from pseudo_hits as ph left join pseudo_hits_annot as pha on pha.hit_idx=ph.idx where pha.hit_idx is null and ph.pair_idx=" + pairIdx + ")");

				pool.executeUpdate("insert ignore into pseudo_hits_annot (select hit_idx,idx,end-start from pseudo_annot join groups on groups.idx=pseudo_annot.grp_idx " + 
						" where type='hit' and groups.proj_idx=" + p1.idx);
				pool.executeUpdate("insert ignore into pseudo_hits_annot (select hit_idx,idx,end-start from pseudo_annot join groups on groups.idx=pseudo_annot.grp_idx " + 
						" where type='hit' and groups.proj_idx=" + p2.idx);
	...
	public void unpackFile(File srcFile, File destDir, TreeSet<String> cleanUp, boolean isSelf, Project p1) throws Exception
	{
		...
		while (sfh.ready())
		{
			...
			if (!h.query.name.equals(qName))
			{
				...
				//log.msg("writing " + destFile.getAbsolutePath());
...
/*	public static void main(String[] args) 
	{
		...
		if (args.length < 2) {
			System.out.println("Usage:  anchors <project1> <project2>\n");
			System.exit(-1);
		}
		...
		try {
			...
			AnchorsMain anchors = new AnchorsMain(pool, new Log("symap.log"), null);
			anchors.run(args[0], args[1]);
	...
	private int scanFile1(AnchorFile file, Project p1, Project p2, boolean skipSelf) throws Exception
	{
		...
		boolean found = false;
		//System.out.println(file.mFile.getAbsolutePath());
		while (fh.ready()) {
			...
			if (line.length() > 0)
			{
				...
				if (hit.query.fileType == FileType.Pseudo) 
				{
					...
					if (hit.query.grpIdx == -1)
					{
						//if (!p1.isUnordered())
						//{
							log.msg("Query not found: " + hit.query.name + " skipping file");
						//}
						fh.close();
						return 0;
					}
				...
				if (hit.target.fileType == FileType.Pseudo) 
				{
					...
					{
						log.msg("Target not found: " + hit.target.name + " skipping file");
						fh.close();
						return 0;
					}
				}
		...
		//System.out.println(totalHits + " scanned; " + totalLargeHits + " large hits; broken to " + totalBrokenHits);
		fh.close();

/*		if (!rawHits.isEmpty()) 
		{
			Group grp1 = p1.getGrpByIdx(rawHits.firstElement().query.grpIdx);
			Group grp2 = p2.getGrpByIdx(rawHits.firstElement().target.grpIdx);
			if (props.getBoolean("do_clustering") )
			{
				createPredictedGenes(rawHits, grp1, grp2, true);
				createPredictedGenes(rawHits, grp2, grp1, false);
			}
		}	*/
	...
	private static int getDefinedOrDefault(int x1, int x2, int defaultVal) {
		if (x1 > 0) return x1;
		if (x2 > 0) return x2;
		return defaultVal;
	}
	
	// This creates clusters along each chromosome from the un-annotated hits, disregarding the other chromosome.
	// The idea is that these might be un-annotated genes, or other conserved things. 
	// These new "annotations" are added to the old, and used for the clustering, which
	// is essential to performance.
/*	public void createPredictedGenes(Vector<Hit> hits, Group grp, Group otherGrp, boolean isQuerySide) throws Exception
	{
		
		HashMap<Integer,Vector<Hit>> bins = new HashMap<Integer,Vector<Hit>>();
		final int maxGap = getDefinedOrDefault(grp.getAvgGeneLength(), otherGrp.getAvgGeneLength(), props.getInt("max_cluster_gap"));
		final int maxGeneLength = getDefinedOrDefault(grp.getMaxGeneLength(), otherGrp.getMaxGeneLength(), props.getInt("max_cluster_size"));
				
		for (Hit h : hits) 
		{
			int start = (isQuerySide ? h.query.start : h.target.start);
			int end   = (isQuerySide ? h.query.end   : h.target.end);

			// Handle long single hits separately 
			// WMN - except, we weren't handling them (4/12/10).
			// We do handle diagonal hits separately and this is the real
			// need, because that is where the super long hits happen. 
			if (end - start > maxGeneLength) 
			{
				//continue;
			}
			// If it overlaps an existing annotation, update the annotation 
			// unless it has gotten too big
			grp.testHitForAnnotOverlapAndUpdate(start, end,maxGeneLength, maxGap);
		}
	}*/
...
//	WMN 9/29/10 we're not using these anymore		
//			// Check annotation includes/excludes, giving the includes preference 
//			checkAnnotIncludes(hit.target,p2,hit);
//			if (p1.isPseudo() && hit.status == HitStatus.Undecided)
//				checkAnnotIncludes(hit.query,p1,hit);
//			
//			if (hit.status == HitStatus.Undecided)
//			{
//				checkAnnotExcludes(hit.target,p2,hit);
//				if (p1.isPseudo() && hit.status == HitStatus.Undecided)
//					checkAnnotExcludes(hit.query,p1,hit);
//			}
//			
//			if (hit.status == HitStatus.Out)
//				continue;
	...
	private boolean scanNextBlatHit(String line, Hit hit, boolean reversed)
	{
		...
		// mdb added 8/14/09 - reduce hit memory footprint
		hit.query.name = hit.query.name.intern();
		hit.target.name = hit.target.name.intern();
		
		hit.matchLen = match;
		hit.pctid = pctid;
		hit.strand = strand.intern(); // mdb added "intern" 8/14/09 - reduce hit memory footprint
	...
	private boolean scanNextMummerHit(String line, Hit hit)
	{
		...
		//String strand = ( (tstart-tend)*(qstart-qend) > 0 ? "+" : "-");					// mdb removed 6/19/09
		String strand = (qstart <= qend ? "+" : "-") + "/" + (tstart <= tend ? "+" : "-"); 	// mdb added 6/19/09

		hit.query.name  = query.intern();	// mdb added "intern" 8/14/09 - reduce hit memory footprint
		hit.target.name = target.intern();	// mdb added "intern" 8/14/09 - reduce hit memory footprint
	...
	// Figure out whether the hit overlaps any annotations in the include/exclude list.
	private void checkAnnotIncludes(SubHit sh, Project proj, Hit hit)
	{		
		Group grp = proj.getGrpByIdx(sh.grpIdx);
		
		int hstart = sh.start;
		int hend = sh.end;
			
		int cs = hstart/annotBinSize;
		int ce = hend/annotBinSize;
		
		for (int c = cs; c <= ce; c++) {
			if (grp.checkAnnotHash(AnnotAction.Include, c)) {
				for (AnnotElem annot : grp.getBinList(AnnotAction.Include,c)) {
					if (Utils.intervalsTouch(hstart,hend,annot.start,annot.end)) {
						hit.status = HitStatus.In;
						proj.incAnnotIncExcCount(AnnotAction.Include, annot.mGT.toString());
						break;
					}
				}
			}
	...
	private void checkAnnotExcludes(SubHit sh, Project proj, Hit hit)
	{
		Group grp =  proj.getGrpByIdx(sh.grpIdx);
		int hstart = sh.start;
		int hend = sh.end;
		...
		for (int c = cs; c <= ce; c++) {
			if (grp.checkAnnotHash(AnnotAction.Exclude, c)) {
				for (AnnotElem annot : grp.getBinList(AnnotAction.Exclude,c)) {
					if (Utils.intervalsTouch(hstart,hend,annot.start,annot.end)) {
						hit.status = HitStatus.Out;
						proj.incAnnotIncExcCount(AnnotAction.Exclude, annot.mGT.toString());
						break;
					}
				}
			}
	...
	public void uploadHit(Hit hit, Project p1, Project p2) throws Exception
	{
		...
		if (p1.isFPC())
		{
//		Group g1 = p1.getGrpByIdx(hit.query.grpIdx);
			Group g2 = p2.getGrpByIdx(hit.target.grpIdx);
//		if ((g1 != null && g1.testHitForAnnotOverlap(hit.query) 
//				        && g1.testHitForAnnotContainment(hit.query, GENE_PAD))
			if (g2 != null && g2.testHitForAnnotOverlap(hit.target))
//					       && g2.testHitForAnnotContainment(hit.target, GENE_PAD)))
			{
				geneOlap = 1; 
			}
		}
...
// mdb removed 7/30/09 - unused	
//	private void incHitTypeCount(Hit hit)
//	{
//		String typestr = hit.query.fileType.toString() + '_' + hit.target.fileType.toString();
//		if (!hitCounts.containsKey(typestr))
//			hitCounts.put(typestr,0);
//		hitCounts.put(typestr,1 + hitCounts.get(typestr));
//	}
//	
//	private void printHitCounts()
//	{
//		for (String type : hitCounts.keySet())
//		{
//			int count = hitCounts.get(type);
//			log.msg("   " + type + ": " + count);
//		}
//	}
	...
	private void hitAnnotations(Project p1, Project p2) throws SQLException
	{
		...
		for (Hit h : newHits)
		{
			...
			count++;
			if (count % 5000 == 0) log.msg(count + " checked");
		}
	...
	private Vector<Hit> getUploadedHits(Project p1) throws SQLException
	{
		...
		if (false) // we actually don't fill this table for fpc // mProj1.mType == ProjType.fpc)
		{
			// Markers
	      	st = "SELECT mrk_hits.idx AS hidx, mrk_hits.start2, mrk_hits.end2" +
	      			" FROM mrk_hits WHERE pair_idx=" + pairIdx;
			...
			// BES
	      	st = "SELECT bes_hits.idx as hidx, bes_hits.start2, bes_hits.end2" +
	      			" FROM bes_hits WHERE pair_idx=" + pairIdx;
		...
		else if (p1.type == ProjType.pseudo)
		{
