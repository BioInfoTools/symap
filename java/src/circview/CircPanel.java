	...
	class Project
	{
		...
		void setArc(int a1, int a2, int cidx)
		{
			...
			for (Group g : grps)
			{
				if (!g.shown) continue;
				cnt++;
				int garc = (int)((arc*((long)g.size)*((long)scaleFactor))/totalSize);
				...
				g.a1 = a;
				g.a2 = b;
				//System.out.println(name + "/" + g.name + ":" + a1 + "," + a2);
				g.cidx = cidx;
				cidx++;
				a = b;
			}
	...
	public void paintComponent(Graphics g) 
	{
        ...
        for (Project p : allProj)
        {
            ...
            if (toScale)
            {
            	...
            	{
            		float shown = (float)p.totalSizeShown();
            		projArc = (int) Math.floor(((360.0 - ((float)nEmpty*projEmptyArc))*shown)/((float)totalSizeShown));
            	}
            }
