...
{
//	private static final String DEFAULT_ALIGN_PARAMS = 
//		"program:promer type1:pseudo type2:pseudo args:\"\"\n" +
//		"program:blat   type1:bes    type2:pseudo args:\"-minScore=30 -minIdentity=70 -tileSize=10 -qMask=lower -maxIntron=10000\"\n" +
//		"program:blat   type1:mrk    type2:pseudo args:\"-minScore=30 -minIdentity=70 -tileSize=10 -qMask=lower -maxIntron=10000\"\n";
	
	// default to nucmer for self-alignment
	private static final String DEFAULT_SELF_ALIGN_PARAMS = 
		"program:nucmer type1:pseudo type2:pseudo args:\"\"\n";
	...
	private boolean checkWritePreproc(File d, int projIdx, String projName, UpdatePool pool, boolean concat, boolean singleFiles) throws Exception
	{
		...
		if (!ok)
		{
			...
			String badChars = Utils.getProjProp(projIdx, "badCharLines", pool);
			boolean hasBadChars = (badChars != null && !badChars.equals("") && !badChars.equals("0"));
			...
			if (needsPreproc)
			{
				//log.msg("Preprocessing " + projName + " sequences "); 
				if (geneMask)
				{
								...
								String projName, File dir, UpdatePool pool, boolean singleFiles) throws Exception
	{
		...
		try
		{
			...
			int cSize = 100000;
			String maskStr = (mask ? " (masked all but genes) " : " ");
			if (mask)
			{
			...
			for (int i = 0; i < groups.size(); i++)
			{
				...
				{
					if (interrupted) break;
					String pfx = Utils.getProjProp(projIdx, "grp_prefix", pool);
	...
	private void buildAlignments2() throws Exception 
	{
		...
		for (FileType t1 : srcList1)
		{
			...
			for (FileType t2 : srcList2)
			{
				...
				//log.msg("Pairing " + d1.getPath() + " and " + d2.getPath());
				...
				//nExist = (new File(resdir)).listFiles(new IsFileFilter()).length;
				int nNotDone = 0;
				...
				//log.msg(nExist + " pairs (" + t1.toString() + "," + t2.toString() + ") have already been completed");
			}
		}
...
/*	private AlignSpecEntry checkAlignSpec(Vector<AlignSpecEntry> mAlignSpec, 
			FileType t1, FileType t2, File f1, File f2)
	{
		AlignSpecEntry ret = null;
		
		// Check the align spec entries for a match. The last match found will govern.
		
		if (t1 == FileType.Pseudo_genemask) t1 = FileType.Pseudo;
		if (t2 == FileType.Pseudo_genemask) t2 = FileType.Pseudo;
		
		int nmatch = 0;
		
		for (AlignSpecEntry a : mAlignSpec)
		{
			if (t1 != a.type1 || t2 != a.type2) continue;
				
			if (!Pattern.compile(a.pat1).matcher(f1.getName()).find()) continue;
			if (!Pattern.compile(a.pat2).matcher(f2.getName()).find()) continue;
		...
		if (nmatch > 1)
			log.msg("WARNING: pair " + f1.getName() + "," + f2.getName() + " matches more than one line in align_params. The last match was used.");
...
*/	
	...
	private static void createPairDir(String mPairDir, ProjType mType1, boolean isSelf)
	{
		...
		//Utilities.checkCreateFile(mPairDir + "/align_params", (isSelf ? DEFAULT_SELF_ALIGN_PARAMS : DEFAULT_ALIGN_PARAMS));
...
/*	
	// The align spec file lets users customize the alignment params and the 
	// set of pairs that are run.
	private void parseAlignSpec(Vector<AlignSpecEntry> mAlignSpec, String mPairDir) throws IOException
	{
		BufferedReader fh = new BufferedReader(new FileReader(new File(mPairDir + "/align_params")));
		while (fh.ready())
		{
			String line = fh.readLine();
			mAlignSpec.add(new AlignSpecEntry(line));
		}
		fh.close();
	}
*/	
...
/*			// If they have gene-masked pseudos, and have not disabled the feature,
			// use those instead of the unmasked.
			File f = new File(seqdir + "/pseudo_genemask");
			if (mUseExtractedGenes 
					&& f.isDirectory() 
					&& f.listFiles(new ExtensionFilter(".seq")).length > 0)
			{
				ret.add(FileType.Pseudo_genemask);
			}
			else
			{
				ret.add(FileType.Pseudo);
