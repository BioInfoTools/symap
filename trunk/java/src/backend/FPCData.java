...
public class FPCData 
{
			...
			Vector<File> besFiles, Vector<File> mrkFiles) throws Exception
	{
		...
		parseFPCFile(f);	
//		Utils.dout(mCtgList.size() + " contigs");
	}
	...
	private void loadData() throws SQLException
	{
		...
		int nbes = mClone2Ctg.keySet().size();
		//mLog.msg(mName + ": " + nbes + " non-singleton BES ");
	}	
	...
	private void parseFPCFile(File f) throws Exception
	{
		...
		//Pattern pABIN = Pattern.compile(".*Anchor_bin.*\"(\\S+)\".*");
		//Pattern pAPOS = Pattern.compile(".*Anchor_bin.*\"(\\S+)\".*");
	...
	private void uploadCloneRemarks() throws Exception
	{
		...
		for (String remark : mCloneRemCounts.keySet())
		{
			...
			mCloneRemCounts.put(remark, idx/*mConn.lastID()*/); // mdb changed 7/1/09
		}
		mpsCloneRem.close();
	}
